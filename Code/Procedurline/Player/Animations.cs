        ...
        public void AddAnimation(PlayerSpriteMode? mode, string id, Sprite.Animation anim) {
            ...
            foreach(MTexture t in anim.Frames) {
                ...
                if(!t.Metadata.TryGetMeta<PlayerAnimMetadata>(out meta)) {
                    Logger.Log(LogLevel.Warn, ProcedurlineModule.Name, $"No metadata for player animation '{id}' frame '{t.AtlasPath}'");
                    continue;
                }
            ...
            if(!mode.HasValue) {
                ...
                if(wildcardAnimations.ContainsKey(id)) Logger.Log(ProcedurlineModule.Name, $"Replacing duplicate wildcard player animation '{id}'");
                ...
                if(modeAnims.ContainsKey(id)) Logger.Log(LogLevel.Warn, ProcedurlineModule.Name, $"Replacing duplicate player animation '{id}' for sprite mode '{mode}'");
                modeAnims[id] = anim;
            }
            ...
            Logger.Log(ProcedurlineModule.Name, $"Added player animation '{id}' for sprite mode '{mode?.ToString() ?? "ANY"}'");
        }
    }
}
